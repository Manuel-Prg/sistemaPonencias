// adminPanel.ts
import { collection, doc, getDocs, query, where, updateDoc, arrayUnion, writeBatch } from "firebase/firestore"
...
export class AdminPanel {
  ...
  private initElements(): void {
    this.elements = {
      ...
      sidebarToggle: document.querySelector("#sidebarToggle") as HTMLElement,
  ...
  private initSidebarCollapse(): void {
    ...
    // Event listener para el toggle en móvil
  ...
  private bindEvents(): void {
    ...
    this.elements.filterBtns.forEach((btn) => btn.addEventListener("click", this.handleFilter.bind(this)))
  ...
  private applyFiltersAndRender(): void {
    ...
    if (searchTerm) {
      filteredUsers = filteredUsers.filter((user) => user.datos?.nombre?.toLowerCase().includes(searchTerm))
    }
